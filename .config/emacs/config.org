...
#+begin_src emacs-lisp :tangle ./early-init.el
(defvar font-name (if (eq system-type 'android)
                      "Iosevka Nerd Font"
                    "Iosevka NF"))
...
* Internet
#+begin_src emacs-lisp :tangle (if (eq system-type 'android) "no" "./init.el")
(use-package mu4e
  :elpaca nil
...
** Language Servers
#+begin_src emacs-lisp :noweb strip-export :tangle (if (eq system-type 'android) "no" "./init.el")
...
(use-package projectile
  ...
  (projectile-project-search-path '("~/Documents/programming" ("~/Documents/programming/repos" . 1))))

(use-package magit
  :bind (("C-c v g" . magit))
  :custom
  (magit-diff-redefine-hunk 'all)
...
#+begin_src emacs-lisp :tangle no
(when (nixos-p)
  (setq elpaca-core-date (get-emacs-build-date))) ;; update this on the next emacs release
