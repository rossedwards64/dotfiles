...
(with-eval-after-load "org"
  ...
  (defun ross/org-presentation-prepare-slide ()
    ""
    (org-overview)
    (org-show-entry)
    (org-show-children))
...
(use-package paredit
  ...
  (((emacs-lisp-mode lisp-mode
     lisp-interaction-mode scheme-mode) . paredit-mode)))

(use-package smartparens
  :config
  (smartparens-global-mode +1)
  (let ((modes '(lisp-mode lisp-interaction-mode
		 emacs-lisp-mode scheme-mode clojure-mode)))
    (dolist (delim '("'" "`"))
      (sp-local-pair modes delim nil :actions :rem))))
