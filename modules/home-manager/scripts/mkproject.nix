...
pkgs.writers.writeGuileBin "mkproject" { } ''
  ...
  l: Project language. Valid options are: c, c++, rust, lisp (SBCL), scheme and clojure.
  p: Project packages.
  d: Project directory.
  h: Print usage.")
  ...
  (define-method (make-project (self <project>))
    ...
    (let ((pipe (open-input-pipe (format #f "GIT_DIR=~a git init -q"
                                         (resolve (list directory ".git"))))))
      (close-pipe pipe))))
  ...
  (define-method (make-project (self <c-project>))
    (next-method))
  ...
  (define-method (make-project (self <rust-project>))
    (let* ((directory (get-directory self))
           (pipe (open-input-pipe (format #f "${pkgs.cargo}/bin/cargo new ~a"
                                          directory))))
      (close-pipe pipe))
    (next-method)
    (make-flake self
                #:vars '((RUST_SRC_PATH pkgs.rust.packages.stable.rustPlatform.rustLibSrc))
                #:shell-hook '("export CARGO_HOME=~/.local/share/cargo"
  ...
  (define-method (make-project (self <clojure-project>))
    (let* ((directory (get-directory self))
           (pipe (open-input-pipe (format #f "${pkgs.leiningen}/bin/lein new ~a --to-dir ~a"
                                        (file-name directory)
                                        directory))))
      (close-pipe pipe))
    (next-method)
